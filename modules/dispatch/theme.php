        ...
        if (!empty($array)) {
            ...
            foreach ($array as $row) {
                if (nv_date('d.m.Y', $row['from_times']) == nv_date('d.m.Y', NV_CURRENTTIME)) {
                    ...
                    if (nv_date('m.Y', $row['from_times']) == nv_date('m.Y', NV_CURRENTTIME) && (nv_date('d', NV_CURRENTTIME) - nv_date('d', $row['from_times']) == 1)) {
                        $xtpl->assign('CLASS', " class=\"r2\"");
                    } else
                        if (nv_date('m.Y', $row['from_times']) == nv_date('m.Y', NV_CURRENTTIME) && (nv_date('d', NV_CURRENTTIME) - nv_date('d', $row['from_times']) == 2)) {
...
/**
 * nv_theme_congvan_search()
 *
 * @param mixed $array_data
 * @return
 */
function nv_theme_congvan_search($array_data)
{
