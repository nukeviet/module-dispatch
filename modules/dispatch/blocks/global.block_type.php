...
if ( ! nv_function_exists( 'nv_type_blocks' ) ) {
	...
	function nv_type_blocks($block_config) 
	{
		...
		$sql = "SELECT id, alias, title,from_time, code, groups_view,file FROM " . NV_PREFIXLANG . "_" . $module . "_document WHERE type IN (" . implode(',', $a_t) . ") ORDER BY date_iss DESC, id DESC LIMIT 0 ," . $block_config['numrow'];
		$result = $db -> query($sql);
